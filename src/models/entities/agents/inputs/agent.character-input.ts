...
4.  **Select Target Slot (Index Range: 0 to limit-1):**
    *   For \`add_memory\` suggestions deemed necessary: If there's an empty slot in \`<YourMemories>\` (indices 0 to ${this.agent.meta.memoryLimit - 1}), use the first available index. If all ${this.agent.meta.memoryLimit} slots are full, identify the *least important* or *most outdated* existing memory (considering the agent's reasoning and current context) and choose its index (between 0 and ${this.agent.meta.memoryLimit - 1}) to overwrite.
    *   For \`add_entity_memory\` suggestions deemed necessary for entity \`key\`: Check \`<YourMemoriesAbout...>\` for that 'key'. If there's an empty slot (indices 0 to ${this.agent.meta.entityMemoryLimit - 1}), use the first available index within that range. If all ${this.agent.meta.entityMemoryLimit} slots (indices 0 to ${this.agent.meta.entityMemoryLimit - 1}) for that entity are full, identify the *least important* or *most outdated* memory *for that specific entity* (considering reasoning and context) and choose its index (between 0 and ${this.agent.meta.entityMemoryLimit - 1}) to overwrite.
5.  **Check for Invalid Existing Memories:** Review the *existing* memories in \'<Input>\'. If any memory slot contains information that is clearly outdated or invalidated by the current interaction context or the agent's reasoning (even without a specific 'add_...' suggestion), plan to update it.
...
7.  **Use Update Tools:** For each necessary update, call the appropriate tool (ensuring the specified index is within the valid range):
