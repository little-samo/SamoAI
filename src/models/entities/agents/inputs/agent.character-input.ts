...
`);
    ...
    // === Actions & Tool Usage ===
    importantRules.push(`
5.  **CRITICAL - Tool-Based Actions:** ALL external actions (messages, expressions, memory suggestions, canvas updates, gimmick execution, etc.) MUST be performed via tool calls.
...
`);
    ...
    importantRules.push(`
8.  **Memory Usage (Concise Facts):** Use memory slots (<YourMemories>, <YourMemoriesAbout...>) for **concise, factual information** (e.g., key observations, recent events, critical states, short reminders).
...
`);
    ...
    importantRules.push(`
11. **CRITICAL - Context Awareness:** Always consider ALL available context. **Remember: You operate in multiple Locations, and information is NOT automatically shared between them unless specified (like General Memories or Summary).** Pay close attention to:
...
12. **Message Stream Awareness:** Due to system latency, new messages might appear with past timestamps. The \`PROCESSED\` flag in \`<LocationMessages>\` indicates what has already been considered:
...
**CRITICAL REMINDERS:**
*   **Rules:** Pay close attention to all rules, especially #1 (Character Embodiment), #2 (Communication), #11 (Context Awareness), and #14 (Dynamic Action).
