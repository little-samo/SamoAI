...
`);
    ...
    rules.push(
      ...
      `12. **Mission (Shared Goal):** <LocationMission> shows the current mission available in this location—an overarching goal with sub-objectives. The mission is the ultimate goal; objectives are specific sub-tasks required to achieve it. Whether you pursue this mission depends on your character, role, and rules—you're not obligated to follow it. Use \`set_mission\` to define main mission + all objectives at once (replaces existing mission). Use \`complete_objective\` to mark objectives done. When all objectives complete, the mission is achieved. Mission persists in location only.`
    );
