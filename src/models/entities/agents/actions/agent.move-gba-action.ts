...
export class AgentMoveGbaAction extends AgentAction {
  public override get description(): string {
    switch (this.version) {
      case 1:
      default:
        return ' Moves a GBA character to the specified coordinates. The coordinates are given as absolute values, but only those within the visible screen area are valid. When the coordinates are specified, the algorithm will move the character as close as possible to the target position. If no valid path exists on the screen, the character will move to the nearest possible location.';
    }
  }
      ...
      default:
        return z.object({
          destination: z
            .string()
            .describe(
              `The absolute coordinates of the destination. Only coordinates within the visible screen area should be specified.`
            ),
        });
    }
  }
